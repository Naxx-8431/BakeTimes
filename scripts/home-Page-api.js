...
document.addEventListener('recipesLoaded', function (event) {
    console.log('Recipes loaded, rendering featured recipes...');
...
function renderFeaturedRecipes(recipes = recipeList) {
    ...
    `;
        ...
        const cardHTML = `<div class="fr-card" data-aos="fade-up" data-aos-delay="${index * 100}">${layout}</div>`;
...
// Initial render (will be empty until recipes load)
renderFeaturedRecipes();
